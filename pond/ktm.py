...
def compile(problem, T, M, init_TL):
    ...
    # verify inputs
    assert isinstance(problem, GroundProblem), "The problem must be ground"
    assert isinstance(T, list) and all(
        [isinstance(t, Literal) for t in T]
    ), "Tags must be a list of Literals"
    assert isinstance(M, list) and all(
        [isinstance(m, tuple) for m in M]
    ), "Merges must be a list of tuples"
    assert isinstance(init_TL, list) and all(
        [isinstance(e, tuple) for e in init_TL]
