...
class Action:
    ...
    def __init__(self, name: str, parameters: List[Tuple[str, str]], precondition: Formula,
        ...
        assert isinstance(parameters, list) and all(
            [isinstance(param, tuple) for param in parameters]
        ), "parameters must be a list of tuples"
        assert (
            isinstance(precondition, Formula) or precondition is None
        ), "precondition must be a Formula object"
        assert (
            isinstance(observe, Predicate) or observe is None
        ), "observe must be a Predicate object or None"
        assert (
            isinstance(effect, Formula) or effect is None
        ), "effect must be a Formula object or None"
    ...
    def is_equal(self, a: Action) -> bool:
        ...
        return (
            self.name == a.name
            and all([sp == ap for sp, ap in zip(self.parameters, a.parameters)])
            and self.precondition == a.precondition
            and self.observe == a.observe
