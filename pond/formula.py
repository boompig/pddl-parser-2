...
class Formula:
    ...
    def __init__(self, name: str, args: List["Formula"]):
        ...
        assert isinstance(args, list) and all(
            [isinstance(arg, Formula) for arg in args]
    ...
    def enforce_normalize(self) -> None:
        ...
        if not isinstance(self, Oneof) and any(
            [isinstance(child, Oneof) for child in self.args]
        ):
            assert False, "Oneof must be on outside, if it exists"

        # 2) the argument to a Not object should be a primitive object
        if isinstance(self, Not) and not all(
            [isinstance(child, Primitive) for child in self.args]
        ):
        ...
        if not isinstance(self, Oneof) and any(
            [isinstance(child, Forall) for child in self.args]
        ):
            assert (
                False
        ...
        if any([isinstance(c, When) for c in self.args]) and not isinstance(
            self, Oneof
        ):
            assert False, "Only thing that can be pulled out of a when is a Oneof"
...
class Forall(Formula):
    ...
    def export(self, lvl: int = 0, sp: str = "  ", untyped: bool = False) -> str:
        ...
        if all([p[1] == Predicate.OBJECT for p in self.params]):
            ...
            param_line = " ".join(["%s - %s" % (p[0], p[1]) for p in self.params])
...
class And(Formula):
    def __init__(self, args: List[Formula]):
        ...
        args = list(filter(lambda x: not isinstance(x, And), args)) + [
            item
            for andarg in filter(lambda x: isinstance(x, And), args)
            for item in andarg.args
        ]
...
class When(Formula):
    ...
    def __init__(self, condition: Formula, result: Formula):
        ...
        assert isinstance(condition, Formula), "Condition must be a Formula object"
        assert isinstance(result, Formula), "Result must be a Formula object"
