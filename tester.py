...
def get_files (d):
    ...
    Outputs:
        p_fname        file name of problem file (without parent path)
    ...
    for f in contents:
        if os.path.isdir (os.path.join (d, f)):
            ...
            d_fname = f
        elif f.startswith ("p") or \
                (d_fname is not None and f.endswith (".pddl")):
            p_fnames.append (f)
...
def batch_parse (parent, parent_name):
    ...
    for f in os.listdir (parent):
        ...
        if os.path.isdir(os.path.join(parent, f)):
            tot += 1
            try:
                result = parse_problem(os.path.join(parent, f),\
                        f, silent=True)
                if len (result) > 0:
...
def batch_normalize (parent, parent_name):
    ...
    for f in os.listdir (parent):
        ...
        if os.path.isdir(os.path.join(parent, f)):
            tot += 1
            try:
                result = normalize_problem(os.path.join(parent, f),\
                        f, silent=True)
                if result:
                    # don't print out successes, just clogs up output
                    # instead keep ratio
...
def export_ground_problem (folder, domain_name, silent=True):
    ...
    for i, p_fname in enumerate (p_fnames):
        ...
        fake_gp = grounder.GroundProblem (fakes["domain"], \
                fakes["problem"], no_ground=True)
...
def export_problem (folder, domain_name, silent=True):
...
def normalize_problem (folder, domain_name, silent=False):
...
def ground_problem (folder, domain_name, silent=False):
...
def find_parent (folder, parents, parent=None):
...
def batch_run (function, parents, parent=None):
    ...
    if parent is None:
        ...
        assert False, \
            "The specified parent is not a valid directory or does not exist"
...
if __name__ == "__main__":
    ...
    l.add_argument ("-p", "--parent", help=\
            "Only list domains for this parent. One of CLG, contff, fsc")
    l.set_defaults (subparser="list")

    tree = sp.add_parser ("tree", help=\
            "Create a tree out of the domain and problem files\
            in the given folder")
    tree.add_argument ("folder")
    tree.set_defaults (subparser="tree")

    pp = sp.add_parser ("parse-problem", help=\
            "Parse a single problem, without its domain")
    pp.add_argument ("problem")
    pp.set_defaults (subparser="parse-problem")

    parse = sp.add_parser ("parse", help=\
            "Parse a problem and domain combination in the given folder")
    parse.add_argument ("folder")
    parse.add_argument ("-n", "--number", type=int, help=\
            "The index of the problem to parse")
    parse.set_defaults (subparser="parse")

    normalize = sp.add_parser ("normalize", help=\
            "Same as parse, except assert that this is also normalized")
    normalize.add_argument ("folder")
    normalize.set_defaults (subparser="normalize")

    ground = sp.add_parser ("ground", help=\
            "Ground the given domain")
    ...
    batch = sp.add_parser ("batch-parse", help=\
            "Parse every domain and problem file that I have. Report results.")
    batch.add_argument ("-p", "--parent", help=\
            parent_string.format("parse"))
    batch.set_defaults (subparser="batch-parse")

    bn = sp.add_parser ("batch-normalize", help=\
            "Same as batch-parse, except verify that the \
            domain and problem are normalized")
    bn.add_argument ("-p", "--parent", help=\
            parent_string.format("normalize"))
    bn.set_defaults (subparser="batch-normalize")

    bg = sp.add_parser ("batch-ground", help=\
            "Same as batch-parse, except also try to ground")
    bg.add_argument ("-p", "--parent", help=\
            parent_string.format("ground"))
    bg.set_defaults (subparser="batch-ground")

    arg_export = sp.add_parser("export", help=\
            "Export given file")
    arg_export.add_argument("folder")
    arg_export.add_argument ("-p", "--parent", help=\
            "Optionally specify a parent in which to find the domain")
    arg_export.add_argument("--verbose", "-v", action='count', help=\
            "Show exported file contents")
    arg_export.set_defaults(subparser="export")

    be = sp.add_parser ("batch-export", help=\
            "Same as batch-parse, except verify that the domain\
            and problem can be exported as PDDL")
    be.add_argument ("-p", "--parent", help=\
            parent_string.format("export"))
    be.set_defaults (subparser="batch-export")

    beg = sp.add_parser ("batch-export-ground", help=\
            "Same as batch-export, except ground problem first")
    beg.add_argument ("-p", "--parent", help=\
            "Optionally, instead of the defaults, export all the domains in the given parent directory")
    beg.set_defaults (subparser="batch-export-ground")

    eg = sp.add_parser ("export-ground", help=\
            "Same as export, except ground problem first")
    eg.add_argument ("folder")
    eg.add_argument("--verbose", "-v", action='count', help=\
            "Show exported file contents")
